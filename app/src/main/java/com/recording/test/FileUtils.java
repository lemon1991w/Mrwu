...
class FileUtils {
    ...
    /**
     * 构造文件处理工具类
     * @param type  0 -- 录音, 1 -- 播放音频文件
     */
    public FileUtils(int type) {
        if(type == 1){ // 音频播放的缓存文件
            // 构造文件根目录
            file = new File(getSdcardPath() + File.separator + "media" + File.separator + "cache");
        }else if(type == 0){ // 录制音频的文件的临时保存文件夹
            // 构造文件根目录
            file = new File(getSdcardPath() + File.separator + "media" + File.separator + "temp");
        }else{
